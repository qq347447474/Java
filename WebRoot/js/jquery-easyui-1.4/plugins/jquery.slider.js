...
(function($) {
	function _1(_2) {
		var _3 = $("<div class=\"slider\">" + "<div class=\"slider-inner\">"
			...
			for (var i = 0; i < aa.length; i++) {
				...
				if (aa[i] != "|") {
					...
					if (_10.mode == "h") {
						_16.css({
									left : _15,
									marginLeft : -Math.round(_16.outerWidth()
											/ 2)
								});
					} else {
						_16.css({
									top : _15,
									marginTop : -Math.round(_16.outerHeight()
											/ 2)
								});
					}
				}
	...
	function _17(_18) {
		...
		_1b.find("div.slider-inner").unbind(".slider").bind("mousedown.slider",
				function(e) {
					if (_19.isDragging || _1a.disabled) {
						return;
					}
					var pos = $(this).offset();
					var _21 = _34(_18, (_1a.mode == "h"
									? (e.pageX - pos.left)
									: (e.pageY - pos.top)));
					_1f(_21);
					_1a.onComplete.call(_18, _1a.value);
	...
	function _23(_24, _25) {
		...
		if (_27.mode == "h") {
			...
			tip.attr("style", _2b + "margin-left:"
	...
	function _34(_35, pos) {
		...
		var _3a = _37.converter.toValue.call(_35, _37.mode == "h"
						? (_37.reversed ? (_39 - pos) : pos)
						: (_39 - pos), _39);
	...
	$.fn.slider.parseOptions = function(_43) {
		var t = $(_43);
		return $.extend({}, $.parser.parseOptions(_43, ["width", "height",
								"mode", {
									reversed : "boolean",
									showTip : "boolean",
									min : "number",
