...
(function($) {
	...
	function _39(_3a, _3b, _3c, _3d) {
		...
		if (_40) {
			...
			var cc1 = _42.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			var cc2 = _43.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			if (!_3d) {
				_40.children = [];
			}
	...
	function _63(_64, _65) {
		...
		var _68 = _67.find("div.datagrid-body tr[node-id=\"" + _65 + "\"]")
	...
	function _9c(_9d, _9e) {
		...
		_29(_9d);
		_9f.total -= 1;
		$(_9d).datagrid("getPager").pagination("refresh", {
					total : _9f.total
				});
		...
		function _a6(_ab) {
			...
			for (var i = 0; i < _ab.length - 1; i++) {
				...
				var cc = tr.next().find("tr.datagrid-row td[field=\""
						+ _a2.treeField + "\"] div.datagrid-cell");
	...
	$.fn.treegrid.methods = {
		...
		update : function(jq, _c2) {
			return jq.each(function() {
						...
						_c3.view.updateRow
								.call(_c3.view, this, _c2.id, _c2.row);
					});
		},
	...
	$.fn.treegrid.parseOptions = function(_c4) {
		return $.extend({}, $.fn.datagrid.parseOptions(_c4), $.parser
						.parseOptions(_c4, ["treeField", {
											animate : "boolean"
										}]));
	};
	var _c5 = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(_c6, _c7, _c8) {
			...
			function _ce(_cf, _d0, _d1) {
				...
				var _d3 = (_d2 ? _d2.children.length : $(_c6)
				...
				for (var i = 0; i < _d1.length; i++) {
					...
					_d4.push("<tr id=\"" + _d8 + "\" node-id=\""
							+ row[_c9.idField] + "\" " + cls + " " + _d7 + ">");
					_d4 = _d4.concat(_cc.renderRow.call(_cc, _c6, _ca, _cf,
							_d0, row));
					_d4.push("</tr>");
			...
			for (var i = 0; i < _e1.length; i++) {
				...
				if (col) {
					...
					if (col.checkbox) {
						...
						cc.push(" name=\"" + _e5 + "\" value=\""
								+ (row[_e5] != undefined ? row[_e5] : "")
								+ "\">");
					} else {
						var val = null;
						if (col.formatter) {
		...
		deleteRow : function(_f6, id) {
			...
			if (_f8) {
				if (_f8.children.length == 0) {
					...
					tr.next("tr.treegrid-tr-tree").remove();
					var _f9 = tr
							.children("td[field=\"" + _f7.treeField + "\"]")
							.children("div.datagrid-cell");
			...
			function _106(rows) {
				rows.sort(function(r1, r2) {
							...
							for (var i = 0; i < _104.length; i++) {
								var sn = _104[i];
								var so = _105[i];
								var col = $(_103).treegrid("getColumnOption",
										sn);
		...
		finder : {
			getTr : function(_110, id, type, _111) {
				...
				if (_111 == 0) {
					...
					if (type == "body") {
						var tr = $("#" + $.data(_110, "datagrid").rowIdPrefix
								+ "-" + _111 + "-" + id);
						if (!tr.length) {
							tr = (_111 == 1 ? dc.body1 : dc.body2)
									.find("tr[node-id=\"" + id + "\"]");
						}
						return tr;
					} else {
						if (type == "footer") {
							return (_111 == 1 ? dc.footer1 : dc.footer2)
									.find("tr[node-id=\"" + id + "\"]");
						} else {
							if (type == "selected") {
								return (_111 == 1 ? dc.body1 : dc.body2)
