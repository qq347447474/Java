...
(function($) {
	...
	$.fn.numberbox.parseOptions = function(_13) {
		...
		return $.extend({}, $.fn.textbox.parseOptions(_13), $.parser
						.parseOptions(_13, ["decimalSeparator",
	...
	$.fn.numberbox.defaults = $.extend({}, $.fn.textbox.defaults, {
		...
		parser : function(s) {
			...
			if (parseFloat(s) != s) {
				if (_1b.prefix) {
					s = $.trim(s.replace(new RegExp("\\" + $.trim(_1b.prefix),
									"g"), ""));
				}
				if (_1b.suffix) {
					s = $.trim(s.replace(new RegExp("\\" + $.trim(_1b.suffix),
									"g"), ""));
				}
				if (_1b.groupSeparator) {
					s = $.trim(s.replace(new RegExp("\\" + _1b.groupSeparator,
									"g"), ""));
				}
				if (_1b.decimalSeparator) {
					s = $.trim(s.replace(new RegExp(
									"\\" + _1b.decimalSeparator, "g"), "."));
				}
				s = s.replace(/\s/g, "");
			}
