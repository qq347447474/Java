...
(function($) {
	...
	function _9(_a) {
		...
		if ($(_a).is(":visible")) {
			...
			if (_c.position == "top" && _e.top < 0) {
				...
				if ((_c.position == "bottom")
						&& (_e.top + _d._outerHeight() > $(window)
								._outerHeight()
								+ $(document).scrollTop())) {
					_e = _f("top");
				}
			...
			if (_e.left < 0) {
				if (_c.position == "left") {
					...
					$(_a).tooltip("arrow").css("left",
							_d._outerWidth() / 2 + _e.left);
					_e.left = 0;
				}
			} else {
				if (_e.left + _d._outerWidth() > $(window)._outerWidth()
						+ $(document)._scrollLeft()) {
					if (_c.position == "right") {
						_e = _f("left");
					} else {
						var _10 = _e.left;
						_e.left = $(window)._outerWidth()
								+ $(document)._scrollLeft() - _d._outerWidth();
						$(_a).tooltip("arrow").css("left",
								_d._outerWidth() / 2 - (_e.left - _10));
					}
				}
			}
		}
		_d.css({
					left : _e.left,
					top : _e.top,
					zIndex : (_c.zIndex != undefined ? _c.zIndex : ($.fn.window
	...
	function _17(_1d, _1e) {
		...
		var cc = typeof _20.content == "function"
				? _20.content.call(_1d)
				: _20.content;
	...
	$.fn.tooltip.parseOptions = function(_29) {
		var t = $(_29);
		var _2a = $.extend({}, $.parser.parseOptions(_29, ["position",
